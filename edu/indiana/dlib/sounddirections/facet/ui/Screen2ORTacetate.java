...
public class Screen2ORTacetate extends EditWindow {

	public JCheckBox offBrandBox, vinegarSyndromeBox,fungusBox, otherProblemsBox;
	...
	public Screen2ORTacetate() {
		...
		JLabel characteristicsLabel = new JLabel("Format: Open Reel Tape-Acetate");
		...
		vinegarSyndromeBox = new JCheckBox("   Vinegar Syndrome");
		vinegarSyndromeBox.setFont(font3);
		vinegarSyndromeBox.setBackground(Main.BROWNCOLOR);
		vinegarSyndromeBox.setBounds(5,75,180,15);
		preservationsJPanel.add(vinegarSyndromeBox);
		vinegarSyndromeBox.addActionListener(new java.awt.event.ActionListener(){
			public void actionPerformed(java.awt.event.ActionEvent e) {
				Main.theData.isDirty = true;
				updateTheData();
				updateTheUI();
		...
		fungusBox.setBounds(5,90,180,15);
		...
		otherProblemsBox.setBounds(5,105,210,15);
		...
		explanationLabel.setBounds(5,125,75,15);
		preservationsJPanel.add(explanationLabel);
		
		explanationText = new JTextField();
		explanationText.setBounds(80,125,255,22);
	...
	public void updateTheData() {
		Main.theData.longPlayAcetate = false;
		Main.theData.doublePlayAcetate = false;
		Main.theData.triplePlayAcetate = false;
		Main.theData.standardAcetate = false;
		Main.theData.unknownDoubleTripleAcetate = false;
		
		if (tapeThickness.getSelectedIndex() == 0) {
			Main.theData.standardAcetate = true;
		}else if (tapeThickness.getSelectedIndex() == 1) {
			Main.theData.longPlayAcetate = true;
		} else if (tapeThickness.getSelectedIndex() == 2) {
			Main.theData.doublePlayAcetate = true;
		} else if (tapeThickness.getSelectedIndex() == 3) {
			Main.theData.triplePlayAcetate = true;
		} else if (tapeThickness.getSelectedIndex() == 4) {
			Main.theData.unknownDoubleTripleAcetate = true;
		}

		Main.theData.offBrand = offBrandBox.isSelected();
		Main.theData.explanationString = explanationText.getText();
		Main.theData.vinegar = vinegarSyndromeBox.isSelected();
		Main.theData.fungus = fungusBox.isSelected();
		Main.theData.visibleMinorAcetate = visibleMinor.isSelected();
		Main.theData.visibleModerateAcetate = visibleModerate.isSelected();
		Main.theData.visibleSevereAcetate = visibleSevere.isSelected();
